...
export class AttendanceService {
  ...
  public async findAllForStudent(student_login: string, subject_id: number) {
    const attendance = await this.attendanceRepository.findAllForStudent(student_login, subject_id);
  ...
  public async post(updateAttendanceDto: UpdateAttendanceDto) {
    const attendance = await this.attendanceRepository.upsert(updateAttendanceDto);
